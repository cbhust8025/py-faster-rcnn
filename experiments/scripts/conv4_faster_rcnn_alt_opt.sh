...
LOG="experiments/logs/conv4_faster_rcnn_alt_opt_${NET}_${EXTRA_ARGS_SLUG}.txt.`date +'%Y-%m-%d_%H-%M-%S'`"
exec &> >(tee -a "$LOG")
...
time ./tools/train_faster_rcnn_alt_opt.py --gpu ${GPU_ID} \
  ...
  --imdb ${TRAIN_IMDB} \
  --cfg experiments/cfgs/conv4_faster_rcnn_alt_opt.yml \
  ${EXTRA_ARGS}
...
time ./tools/test_net.py --gpu ${GPU_ID} \
  --def models/${PT_DIR}/${NET}/conv4_faster_rcnn_alt_opt/conv4_faster_rcnn_test.pt \
  --net ${NET_FINAL} \
  --imdb ${TEST_IMDB} \
  --cfg experiments/cfgs/conv4_faster_rcnn_alt_opt.yml \
  ${EXTRA_ARGS}
